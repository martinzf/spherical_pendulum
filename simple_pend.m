...
% It's easier to find solutions for h >= g/l
    ...
    if h < g/l
        ...
        e0 = sign(d_alpha0)*(ellipke(k^2) - ellipticF(asin(sin(alpha0/2)/k), k^2)); 
        [~,cn,dn] = ellipj(sqrt(g/l)*t-e0, k^2); % Expression evaluated at (t-t0)
        cd = cn./dn;
        alpha = 2*asin(k*cd);
    % Bounded
    elseif h == g/l 
        e0 = atanh(sin(alpha0/2));
        alpha = 2*asin(tanh(sign(d_alpha0)*sqrt(g/l)*t+e0)); % Expression evaluated at (t-t0)
    % Unbounded
    else
        m = 2*g/l/(h+g/l);
        e0 = sign(d_alpha0)*ellipticF(sin(alpha0/2),m);
        [sn,cn,~] = ellipj(sqrt((h+g/l)/2)*t+e0, m); % Expression evaluated at (t-t0)
        alpha = 2*sign(d_alpha0)*sign(cn).*asin(sn);
    end
end